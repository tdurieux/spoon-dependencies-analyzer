...
public class MethodDependencyAnalyzerTest {

    private github.tdurieux.dependencyAnalyzer.DependencyAnalyzer dependencyAnalyzer;
    private DependencyGraph graph;

    public MethodDependencyAnalyzerTest() throws Exception {
    ...
    @Test
    public void detectsMethodTest() {
        Map<DependencyNode, List<DependencyNode>> nodes = graph.getUsedNodes();

        ArrayList<DependencyNode> keys = new ArrayList<DependencyNode>(nodes.keySet());

        assertTrue(keys.contains(createFakeDependencyNode("java.lang.Integer")));
        DependencyNode object = keys.get(keys.indexOf(createFakeDependencyNode("github.tdurieux.testProject.entity.User::isInteger(java.lang.Object)")));
        assertEquals(DependencyNode.Type.METHOD, object.getType());
        assertFalse(object.isExternal());
        object = keys.get(keys.indexOf(createFakeDependencyNode("github.tdurieux.testProject.entity.User::User()")));
        assertEquals(DependencyNode.Type.CONSTRUCTOR, object.getType());
        assertFalse(object.isExternal());
    }
    ...
    private DependencyNode createFakeDependencyNode(String qualifiedName) {
        ...
        return new DependencyNodeImpl(qualifiedName, qualifiedName,
                DependencyNode.Type.METHOD, isExternal, isInternal,
